...
de ta réponse), sans balises Markdown. Format EXACT :
{
  ...
  "overall_reasoning": "synthèse courte de ta lecture de l'actu et de ta stratégie du jour"
}
...
- "HOLD" : ne rien faire.
- Chaque "reason" doit citer le catalyseur concret qui justifie l'action.
...
def run_once() -> None:
    ...
    log_event({"type": "end", "equity_after": float(trading.get_account().equity)})
